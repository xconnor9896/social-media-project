...
const getUsernameAvailable = async (req, res) => {
  ...
  try {
    if (username.length < 1)
      return res.status(401).send(`This username is too short!`);
    ...
    if (user)
      return res.status(401).send(`Sorry, this username is already taken!`);
...
const createUser = async (req, res) => {
  ...
  if (password.length < 6)
    return res
      .status(401)
      .send(
        "Your password is too short! It has to be at least six characters."
      );
